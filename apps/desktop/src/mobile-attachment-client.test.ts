...
describe('createMobileAttachmentClient', () => {
  it('stages a file then sends its server reference to the same runtime session', async () => {
    const request = vi.fn(async (method: string) =>
      method === 'file.attach' ? { attached: true, ref_text: '@file:uploaded.txt' } : { status: 'accepted' }
    )

    const client = createMobileAttachmentClient({ request })

    await client.submit('runtime-42', 'Please read this', [
      { dataUrl: 'data:text/plain;base64,SGVsbG8=', name: 'notes.txt' }
    ])
