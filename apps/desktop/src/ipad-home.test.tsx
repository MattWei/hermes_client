...
describe('IpadHome', () => {
  ...
  it('requires an explicit risk confirmation for a validated LAN connection', () => {
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'Backend address' }), {
      target: { value: 'http://10.112.141.190:9119' }
    })
    ...
    fireEvent.click(
      screen.getByRole('checkbox', { name: 'I understand this LAN connection uses unencrypted HTTP and WS' })
    )
  ...
  it('rejects a hostname in LAN mode', () => {
    ...
    fireEvent.change(screen.getByRole('combobox', { name: 'Connection mode' }), { target: { value: 'lan' } })
    fireEvent.change(screen.getByRole('textbox', { name: 'Backend address' }), {
      target: { value: 'http://hermes-dashboard.local' }
    })
    fireEvent.change(screen.getByLabelText('LAN session token'), { target: { value: 'not-persisted' } })
    fireEvent.click(
      screen.getByRole('checkbox', { name: 'I understand this LAN connection uses unencrypted HTTP and WS' })
  ...
  it('reads the saved LAN token and connects only through the supplied gateway adapter', async () => {
    ...
    fireEvent.change(screen.getByRole('combobox', { name: 'Connection mode' }), { target: { value: 'lan' } })
    fireEvent.change(screen.getByRole('textbox', { name: 'Backend address' }), {
      target: { value: 'http://10.10.10.10:9119' }
    })
    ...
    render(
      <IpadHome
        ...
        credentialStore={{
          clear: vi.fn(),
          read: vi.fn(),
          readLanSessionToken: vi.fn(async () => 'stored-lan-token'),
          save: vi.fn(),
          saveLanSessionToken: vi.fn()
        }}
      />
  ...
  it('resumes a selected stored session and renders its Desktop-hydrated history', async () => {
    const request = vi.fn(async (method: string) =>
      method === 'session.resume' ? { session_id: 'runtime-2', session_key: 'stored-2' } : { status: 'accepted' }
    )
    ...
    render(
      <IpadHome
        connectGateway={vi.fn(async () => ({ close: vi.fn(), on: vi.fn(() => vi.fn()), request }))}
        createRestClient={vi.fn(() => ({ get }))}
        credentialStore={{
          clear: vi.fn(),
          read: vi.fn(),
          readLanSessionToken: vi.fn(async () => 'stored-lan-token'),
  ...
  it('creates a session then submits and renders its scoped streamed reply', async () => {
    ...
    await waitFor(() =>
      expect(request).toHaveBeenCalledWith('prompt.submit', { session_id: 'runtime-42', text: 'Hello from Android' })
    )
    ...
    act(() =>
      handlers.get('approval.request')?.({
        session_id: 'runtime-42',
        payload: { command: 'safe command', request_id: 'approval-1' }
      })
    )
    ...
    act(() =>
      handlers.get('tool.start')?.({ session_id: 'other-session', payload: { name: 'ignored', tool_id: 'tool-other' } })
    )
    act(() =>
      handlers.get('tool.start')?.({ session_id: 'runtime-42', payload: { name: 'terminal', tool_id: 'tool-1' } })
    )
    expect(screen.getByText('terminal · running')).toBeTruthy()
    expect(screen.queryByText('ignored · running')).toBeNull()
    act(() =>
      handlers.get('tool.complete')?.({ session_id: 'runtime-42', payload: { name: 'terminal', tool_id: 'tool-1' } })
    )
