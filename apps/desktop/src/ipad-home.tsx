...
import {
  ...
  loadMobileSessionHistory,
  type MobileGatewayRequest,
  type MobileSessionListItem,
  type MobileSessionRest
} from './mobile-session-client'
...
interface IpadHomeProps {
  ...
  ) => Promise<MobileGatewayRequest & MobilePromptGateway & { close(): void }>
...
function toDataUrl(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () =>
      typeof reader.result === 'string' ? resolve(reader.result) : reject(new Error(`Could not read ${file.name}`))
...
export function IpadHome({
  credentialStore = mobileCredentialStore,
  connectGateway = connectMobileGateway,
  createRestClient = createMobileGatewayRestClient
}: IpadHomeProps) {
  ...
  const [gateway, setGateway] = useState<
    | (MobileGatewayRequest &
        MobilePromptGateway & { close(): void; onState?: (handler: (state: string) => void) => () => void })
    | null
  >(null)
  ...
  useEffect(() => {
    ...
    return gateway.on('session.replay.gap', raw => {
      ...
      void loadMobileSessionHistory(rest, expected.storedSessionId)
        .then(history => {
          if (
            activeSessionRef.current.runtimeSessionId === expected.runtimeSessionId &&
      ...
      setMessages(current => {
        ...
        if (event.complete) {
          if (assistantIndex < 0) {
            return [
              ...current,
              { id: `mobile-assistant-${Date.now()}`, parts: [assistantTextPart(event.text)], role: 'assistant' }
            ]
          }
        ...
        if (assistantIndex < 0) {
          return [
            ...current,
            {
              ...
              parts: [assistantTextPart(event.text)],
              pending: true,
              role: 'assistant'
            }
          ]
        ...
        return current.map((message, index) =>
          index === assistantIndex ? { ...message, parts: appendAssistantTextPart(message.parts, event.text) } : message
        )
      })
    })
  }, [gateway, sessionId])
  ...
  async function startNewSession() {
    ...
    try {
      ...
      if (storedSessionId) {
        setSessions(current =>
          current.some(item => item.id === storedSessionId)
            ? current
  ...
  async function submitMessage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const attachments: MobileFileAttachment[] = await Promise.all(
        pendingFiles.map(async file => ({ dataUrl: await toDataUrl(file), name: file.name }))
      )
  ...
  return (
    <main className="min-h-dvh overflow-x-hidden bg-[var(--ui-bg-primary)] px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(1.5rem,env(safe-area-inset-top))] text-[var(--ui-text-primary)]">
      <div className="mx-auto flex min-h-[calc(100dvh-3rem)] max-w-3xl flex-col">
        ...
        <section aria-labelledby="welcome-title" className="flex flex-1 flex-col justify-center py-12">
          <p className="mb-3 font-mono text-xs tracking-[0.18em] text-[var(--ui-text-tertiary)]">
          ...
          <p className="mt-5 max-w-lg text-base leading-7 text-[var(--ui-text-secondary)]">
            This first build keeps the Hermes experience on your mobile device while your agent stays on another
            computer.
          </p>
          {emulatorLoopbackEnabled && (
            <p className="mt-4 w-fit rounded-lg border border-amber-500/50 px-3 py-2 font-mono text-xs text-amber-700 dark:text-amber-300">
          ...
          <div className="grid gap-3 rounded-2xl bg-[var(--ui-bg-quaternary)] p-4">
            <label className="grid gap-1 text-sm">
              ...
              <select
                ...
                className="rounded-lg bg-[var(--ui-bg-primary)] p-2"
                onChange={event => {
            ...
            <label className="grid gap-1 text-sm">
              ...
              <input
                ...
                className="rounded-lg bg-[var(--ui-bg-primary)] p-2"
                onChange={event => setBackendAddress(event.target.value)}
                placeholder={mode === 'lan' ? 'http://192.168.1.10:9119' : 'https://hermes.example.com'}
            ...
            {mode === 'lan' && (
              <>
                ...
                <label className="flex gap-2 text-sm text-[var(--ui-text-secondary)]">
                  <input
                    aria-label="I understand this LAN connection uses unencrypted HTTP and WS"
                    checked={lanRiskConfirmed}
                    onChange={event => setLanRiskConfirmed(event.target.checked)}
                    type="checkbox"
                  />
            ...
            <button
              className="w-fit rounded-xl bg-[var(--ui-text-primary)] px-4 py-2 text-sm font-medium text-[var(--ui-bg-primary)] disabled:opacity-40"
              disabled={mode === 'lan' && (!lanRiskConfirmed || !lanToken)}
              type="submit"
            >
              Validate connection
            </button>
            {mode === 'lan' && (
              <button
                className="w-fit rounded-xl border border-(--ui-stroke-tertiary) px-4 py-2 text-sm font-medium disabled:opacity-40"
                disabled={!lanRiskConfirmed || !lanToken}
                onClick={() => void saveLanToken()}
                type="button"
              >
                Save LAN token
              </button>
            ...
            {mode === 'lan' && (
              <button
                className="w-fit rounded-xl border border-(--ui-stroke-tertiary) px-4 py-2 text-sm font-medium disabled:opacity-40"
                disabled={Boolean(gateway) || !lanRiskConfirmed || !lanToken}
                onClick={() => void connectLanGateway()}
                type="button"
              >
                Connect LAN gateway
              </button>
            ...
            {gateway && (
              <button
                className="w-fit rounded-xl border border-(--ui-stroke-tertiary) px-4 py-2 text-sm font-medium"
        ...
        {sessions.length > 0 && (
          <section aria-label="Saved sessions" className="flex flex-col gap-2 py-4">
            {sessions.map(session => (
              <button
                aria-label={`Open ${session.title || 'Untitled session'}`}
                className="rounded-xl border border-(--ui-stroke-tertiary) p-3 text-left"
                key={session.id}
        ...
        {approval && (
          <section aria-label="Approval request" className="rounded-xl border border-amber-500/50 p-3">
            ...
            <div className="mt-3 flex gap-2">
              <button onClick={() => void respondToApproval('once')} type="button">
                Allow once
              </button>
              <button onClick={() => void respondToApproval('deny')} type="button">
                Deny
              </button>
            </div>
          </section>
        ...
        {toolActivities.length > 0 && (
          <section aria-label="Tool activity" className="flex flex-col gap-2 py-4">
            <p className="font-mono text-xs tracking-[0.14em] text-[var(--ui-text-tertiary)]">TOOL ACTIVITY</p>
            {toolActivities.map(activity => (
              <p className="rounded-xl border border-(--ui-stroke-tertiary) px-3 py-2 text-sm" key={activity.id}>
        ...
        {messages.length > 0 && (
          <section aria-label="Conversation" className="flex flex-1 flex-col gap-3 py-6">
            {messages.map(message => (
              <article
                className={
                  message.role === 'user'
                    ? 'self-end rounded-2xl bg-[var(--ui-bg-quaternary)] p-3'
                    : 'self-start rounded-2xl border border-(--ui-stroke-tertiary) p-3'
                }
                key={message.id}
              >
        ...
        <form className="border-t border-(--ui-stroke-tertiary) pt-4" onSubmit={submitMessage}>
          {pendingFiles.length > 0 && (
            <div aria-label="Pending attachments" className="mb-2 flex flex-wrap gap-2">
              {pendingFiles.map((file, index) => (
                <button
                  className="rounded-lg border border-(--ui-stroke-tertiary) px-2 py-1 text-sm"
                  key={`${file.name}-${index}`}
          ...
          <input
            ...
            className="sr-only"
            disabled={!sessionId}
            onChange={event => setPendingFiles(current => [...current, ...Array.from(event.target.files ?? [])])}
            type="file"
          />
          ...
          <div className="flex items-end gap-3 rounded-2xl bg-[var(--ui-bg-quaternary)] p-3">
            <textarea
              ...
              placeholder={sessionId ? 'Message Hermes' : 'Create a session to start a conversation'}
              rows={2}
              value={draft}
            />
            <button
              aria-label="Send message"
              className="shrink-0 rounded-xl bg-[var(--ui-text-primary)] px-4 py-2 text-sm font-medium text-[var(--ui-bg-primary)] disabled:opacity-40"
              disabled={!sessionId || (!draft.trim() && pendingFiles.length === 0)}
              type="submit"
            >
              Send
            </button>
